...
function indekseGoreFenomen(dizi,sayi) {
  const result =`${sayi}. indekste bulunan fenomen: ${dizi[sayi].profile}` ;
  return result
}
...
console.log(fenomenSil(fenomenler,2)[2].profile)
...
function fenomenEkle(dizi,number,profile,followers,p0sts,platform) {
  let yeniFenomenler = [...dizi];
      const newObject ={
        "number": number,
    "profile": profile,
    "followers": followers,
...
function enFenomenler(fenomenler) {
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    if(fenomenler[i].followers> 100000000){
      result.push(fenomenler[i].profile)
    }
...
for (let i= 0; i< fenomenler.length ; i++) {
  if(fenomenler[i].platform==platform){
    if(fenomenler[i].posts >= max_post){
      max_post = fenomenler[i].posts
      max_index = i
    }
  }
}
return fenomenler[max_index].profile;
